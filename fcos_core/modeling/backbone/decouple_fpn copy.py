...
from fcos_core.layers.layer_misc import SACLikeConv, Hsigmoid
...
class FPNAddFuseModule(nn.Module):
    def __init__(self, channels, num_convs=None, identified=False): #channels, num_convs=None):
        super().__init__()
...
class DEFuseModule(nn.Module):
    def __init__(self, channels, num_convs=None):
        super(DEFuseModule, self).__init__()

        self.sac_conv = SACLikeConv(channels, channels, 3, 1, 1)
        ...
        if self.use_tower:
            self._init_weight([self.convs, self.weight])
        else:
            self._init_weight([self.weight, ])

    def _init_weight(self, m_list):
        def _init(m):
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, a=1)
                if m.bias is not None:
    ...
    def forward(self, agg, appr):
        x = self.sac_conv(agg) + appr

        if self.use_tower:
            x = self.convs(x)
        
        weight = self.weight(x)
        w1, w2 = list(weight.chunk(2, dim=1))
        x = w1 * x + w2 * appr
...
class DecoupleFPN(nn.Module):
    def __init__(self, in_channels_list,
        ...
        agg_appr_fuses=[True, True, True, True, True], # 0,1,2,3,4
        idns=[True, True, True, True, True], # 0,1,2,3,4
        fuse_conv_num = [1, 1, 1, 0, 0],
        use_stride_4=False,
        use_c5=False
    ):
        ...
        for idx, in_channel in enumerate(in_channels_list):
            ...
            if self.agg_appr_fuses[ind_fuse]:
                fuse_module = FPNAddFuseModule(
                    out_channels, 
                    num_convs=fuse_conv_num[ind_fuse], 
                    identified=idns[ind_fuse])
                # fuse_module = DEFuseModule(out_channels, num_convs=fuse_conv_num[ind_fuse])
    ...
    def forward(self, features):
        ...
        if self.use_stride_4:
            ...
            if self.num_outs > len(features)-1:
                # add
                if self.use_c5:
                    assert False, "remove c5"
                    decouples.extend(self.top_decouples(features[-1]))
                else:
                    top_feat = decouples[-1].agg + decouples[-1].appr
                    decouples.extend(self.top_decouples(top_feat))
